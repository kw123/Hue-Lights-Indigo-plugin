...
class Plugin(indigo.PluginBase):
			...
			if device.deviceTypeId == "hueAttributeController":
				if device.id not in self.controlDeviceList:
					try:
						...
						self.exceptionHandler(30, e, extraText="Attribute Control device definition cannot be displayed ")
			...
			# Hue Groups
				...
				if device.id not in self.deviceList:
					try:
						...
						self.exceptionHandler(30, e, extraText="Hue Group device definition cannot be displayed ")
	...
	def runConcurrentThread(self):
		...
		try:
			while True:
				...
				if time.time() - lastTimeForAll >= max(30., goAllRefresh / self.timeScaleFactor):
					lastTimeForAll = time.time()
					self.updateAllHueLists()
	...
	def autocreateNewDevices(self, valuesDict,y):
			...
			for hubNumber in self.hueConfigDict:
				...
				for theID in theDict:
					...
					for typId in kmapHueTypeToIndigoDevType:
						if theDict[theID]["type"].find(typId) ==0:
							deviceTypeId = kmapHueTypeToIndigoDevType[typId][0]
							break

					if deviceTypeId  == "":
						self.indiLOG.log(20,u"autocreateNewDevices lights  hub:{:>2s}; id:{:>3s}, type:{:25s}      not supported".format(hubNumber, theID, theDict[theID]["type"]))
						continue

					found = False
					for dev in indigo.devices.iter(self.pluginId):
						if dev.deviceTypeId != deviceTypeId: continue
						if str(theID) != str(dev.pluginProps.get('bulbId', "xx")): continue
						found = True
						self.indiLOG.log(20,u"autocreateNewDevices lights  hub:{:>2s}; id:{:>3s}, type:{:25s}      already exists".format(hubNumber, theID, theDict[theID]["type"]))
						break
					
					if not found:
						name = u"Hue_light_{}_{}_{}".format(hubNumber, theID, theDict[theID]["name"])
						address = u"{} (Lid:{}-{})".format(self.ipAddresses[hubNumber], hubNumber, theID )
						props ={}
						...
						self.indiLOG.log(20,u"autocreateNewDevices lights  hub:{:>2s}; id:{:>3s}, type:{:25s}      create         {:}".format( hubNumber, theID, theDict[theID]["type"],   name))
			...
			for hubNumber in self.hueConfigDict:
				...
				for theID in theDict:
					...
					if deviceTypeIdCandidates == []:
						self.indiLOG.log(20,u"autocreateNewDevices sensors hub:{:>2s}; id:{:>3s}; type:{:25s}      not supported".format(hubNumber, theID, theDict[theID]["type"]))
						continue
					found = ""
					...
					for dev in indigo.devices.iter(self.pluginId):
						if dev.deviceTypeId != deviceTypeId: continue
						if str(theID) != str(dev.pluginProps.get('sensorId', "xx")): continue
						found = True
						self.indiLOG.log(20,u"autocreateNewDevices sensors hub:{:>2s}; id:{:>3s}; type:{:25s}      already exists".format(hubNumber, theID, theDict[theID]["type"]))
						break
					
					if not found:
						name = u"Hue_sensor_{}_{}_{}".format(hubNumber, theID, theDict[theID]["name"])
						address = u"{} (Sid:{}-{})".format(self.ipAddresses[hubNumber], hubNumber, theID )
						props ={}
						props['hubNumber'] = hubNumber
						...
						self.indiLOG.log(20,u"autocreateNewDevices sensors hub:{:>2s}; id:{:>3s}; type:{:25s}      create         {:}".format( hubNumber, theID, theDict[theID]["type"], name))
						dev = indigo.device.create(
							protocol		= indigo.kProtocol.Plugin,
							address			= address,
							name			= name,
		...
		if valuesDict['createGroups']:
			...
			for hubNumber in self.hueConfigDict:
				...
				for theID in theDict:
					...
					for dev in indigo.devices.iter(self.pluginId):
						if dev.deviceTypeId != deviceTypeId: continue
						if hubNumber != dev.pluginProps.get('hubNumber', "xx"): continue
						if str(theID) != str(dev.pluginProps.get('groupId', "xx")): continue
						found = True
						self.indiLOG.log(20,u"autocreateNewDevices groups  hub:{:>2s}; id:{:>3s}; type:{:25s}      already exists".format(hubNumber, theID, theDict[theID]["type"]))
						break

					if not found:
						name = u"Hue_group_{}_{}_{}".format(hubNumber, theID, theDict[theID]["name"])
						address = u"{} (Gid:{}-{})".format(self.ipAddresses[hubNumber], hubNumber, theID )
						props ={}
						props['hubNumber'] = hubNumber
						props['groupId'] = theID
						props['groupLights'] = ""
						props['groupLightsDisabler'] =False
						props['rate'] = "1"
						props['noOnRampRate'] = False
						props['noOffRampRate'] = False
						props['savedBrightness'] = "0"
						props['nameOnHub'] = ""
						props['type'] = ""
						props['groupClass'] = ""
						props['logChanges'] = True
						...
						props['WhiteTemperatureMin'] = "2000"
						props['WhiteTemperatureMax'] = "6500"
						props['SupportsRGBandWhiteSimultaneously'] = True

						self.indiLOG.log(20,u"autocreateNewDevices groups  hub:{:>2s}; id:{:>3s}; type:{:25s}      create         {:}".format( hubNumber, theID, theDict[theID]["type"], name))
						dev = indigo.device.create(
							protocol		= indigo.kProtocol.Plugin,
							address			= address,
							name			= name,
		...
		self.indiLOG.log(20,u"autocreateNewDevices Lights  --  #of Indigo devices created:{} ".format(createdLights))
		self.indiLOG.log(20,u"autocreateNewDevices Sensors --  #of Indigo devices created:{} ".format(createdSensors))
		self.indiLOG.log(20,u"autocreateNewDevices Groups  --  #of Indigo devices created:{} ".format(createdGroups))
		return
	...
	def commandToHub_HTTP(self, hubNumber, cmd, errorsDict={}, errDict1="", errDict2=""):
		...
		try:
			...
			errorText = self.doErrorLog(u"Failed to connect to the Hue bridge at {} after {} seconds. - Check that the bridge is connected and turned on.".format(ipAddress, kTimeout))
			errorsDict[errDict1] = errorText
			errorsDict[errDict2] += errorsDict[errDict1]
			return (False, "", errorsDict)
		except requests.exceptions.ConnectionError:
			errorText = self.doErrorLog(u"Failed to connect to the Hue bridge at {}. - Check that the bridge is connected and turned on.".format(ipAddress))
			errorsDict[errDict1] = errorText
			errorsDict[errDict2] += errorsDict[errDict1]
			return (False, "", errorsDict)
		...
		# -- Hue Device Attribute Controller (Virtual Dimmer Device) --
			...
			if valuesDict.get('defaultOnLevel', "") != "":
				try:
					...
					isError = True
					errorsDict['defaultOnLevel'] = u"The Default On Level must be a whole number between 1 and 100. Error: {}  @line#:{})".format(e,sys.exc_info()[2].tb_lineno)
					errorsDict['showAlertText'] += errorsDict['defaultOnLevel'] + "\n\n"
	...
	def selHubNumberCallback(self, valuesDict):
		...
		if gwAction == "delete":
			...
			if self.hubNumberSelected in self.hueConfigDict:
				del self.hueConfigDict[hubNumber]
	...
	def validatePrefsConfigUi(self, valuesDict):
		if self.decideMyLog(u"EditSetup"): self.indiLOG.log(10,u"Starting validatePrefsConfigUi.")
		...
		if not isError:
			try:
				...
				self.exceptionHandler(30, e, extraText="validatePrefsConfigUi: Connection error")
				isError = True
				errorsDict['address'] = u"Connection error. Please check the IP address and ensure that the Indigo server and Hue bridge are connected to the network."
				errorsDict['showAlertText'] += errorsDict['address'] + u"\n\n"
	...
	def actionControlDimmerRelay(self, action, device):
		try:
			...
			self.exceptionHandler(20, e, extraText=u"Starting actionControlDimmerRelay for device {}. (Unable to display action or device data".format(device.name))
		...
		if device.deviceTypeId == "hueBulb":
			...
			if command == indigo.kDeviceAction.TurnOn:
				try:
					...
					self.exceptionHandler(30, e, extraText="device on: Unable to display action data")
				# Turn it on.
			...
			##### TURN OFF #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device off: Unable to display action data")
			...
			##### TOGGLE #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device toggle: Unable to display action data")
				if currentOnState :
					# It's on. Turn it off.
					self.doOnOff(device, False)
				else:
			...
			##### SET BRIGHTNESS #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device set brightness: Unable to display action data")
			...
			##### BRIGHTEN BY #####
				...
				try:
					...
					self.indiLOG.log(30,u"device increase brightness by: (Unable to display action data due to error: {}  line#:{})".format(e,sys.exc_info()[2].tb_lineno))
			...
			##### DIM BY #####
				...
				try:
					...
					self.indiLOG.log(30,u"device decrease brightness by: (Unable to display action data due to error: {}  line#:{})".format(e,sys.exc_info()[2].tb_lineno))
					self.exceptionHandler(30, e, extraText="device decrease brightnes: Unable to display action data")
				brightnessLevel = currentBrightness - action.actionValue
			...
			##### SET COLOR LEVELS #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device request status: Unable to display action data")
		...
		if device.deviceTypeId == "hueAmbiance":
			...
			##### BRIGHTEN BY #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device increase brightness by: Unable to display action data ")
				brightnessLevel = currentBrightness + action.actionValue
				if brightnessLevel > 100:
					brightnessLevel = 100
				# Save the new brightness level into the device properties.
			...
			##### DIM BY #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device decrease brightness by: Unable to display action data ")
				brightnessLevel = currentBrightness - action.actionValue
				if brightnessLevel < 0:
					brightnessLevel = 0
				# Save the new brightness level into the device properties.
		...
		#
			...
			##### SET COLOR LEVELS #####
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="device set color: Unable to display action data ")
		...
		#
			...
			if rate == "":
				...
				try:
					...
					self.exceptionHandler(30, e, extraText="Invalid rate ")
					rate = -1
	...
	def actionControlSensor(self, action, device):
		try:
			...
			self.exceptionHandler(30, e, extraText=u"Starting actionControlSensor for device {}. Unable to display action or device data".format(device.name))
	...
	def calcRgbHexValsFromHsbLevels(self, valuesDict):
		if self.decideMyLog(u"Loop"): self.indiLOG.log(10,u"Starting calcRgbHexValsFromHsbLevels.")
	...
	def bulbListGenerator(self, filter="", valuesDict=None, typeId="", targetId=0):
		...
		for hubNumber in hubNumbers:
			for bulbId, bulbDetails in sorted(self.hueConfigDict[hubNumber]['lights'] .items(), key = lambda x: x[1]['name']):
				if typeId == "":
		...
		# Debug
		if self.decideMyLog(u"EditSetup"): self.indiLOG.log(10,u"bulbListGenerator: Return bulb list is {}".format(xList))
	...
	def groupListGenerator(self, filter="", valuesDict=None, typeId="", targetId=0):
		...
		for hubNumber in hubNumbers:
			for groupId, groupDetails in sorted(self.hueConfigDict[hubNumber]['groups'] .items(), key = lambda x: int(x[0])):

				xList.append([groupId, u"{}-{}:{}".format(hubNumber, groupId, groupDetails['name'])])

		# Debug
		if self.decideMyLog(u"EditSetup"): self.indiLOG.log(10,u"groupListGenerator: Return group list is {}".format(xList))
	...
	def usersListGenerator(self, filter="", valuesDict=None, typeId="", deviceId=0):
		...
		for hubNumber in hubNumbers:
			if self.hueConfigDict[hubNumber]['users'] != None:
	...
	def scenesListGenerator(self, filter="", valuesDict=None, typeId="", deviceId=0):
		...
		for hubNumber in hubNumbers:
			...
			if self.hueConfigDict[hubNumber]['scenes'] != None:
	...
	def groupLightsListGenerator(self, filter="", valuesDict=None, typeId="", deviceId=0):
		...
		for hubNumber in hubNumbers:
			...
			if groupId != "":
				...
				# If the groupId is 0, then the all lights group was selected.
				if groupId == "0":
					groupLights = self.hueConfigDict[hubNumber]['lights'].keys()
				else:
					groupLights = self.hueConfigDict[hubNumber]['groups'][groupId]['lights']
	...
	def gwListGenerator(self, filter="", valuesDict=None, typeId="", targetId=0):
		# Used in actions and device configuration windows that need a list of sensor devices.
		if self.decideMyLog(u"EditSetup"): self.indiLOG.log(10,u"Starting gwListGenerator.\n  filter: {}\n  valuesDict: {}\n  typeId: {}\n  targetId: {}".format(filter, valuesDict, typeId, targetId))
		...
		# Debug
		if self.decideMyLog(u"Init"): self.indiLOG.log(10,u"gwListGenerator: Return hubNumber list is {}".format(xList) )
			...
			for statesDict in state:
				...
				if newValue != device.states.get(stateKey, None):
					try:
						...
						if self.decideMyLog(u"UpdateIndigoDevices"): self.exceptionHandler(30, e, extraText=u"updateDeviceState: Updating device \"{}\" state: Unable to display state".format(device.name))
		...
		# If state wasn't a list, treat it like a string and just update 1 device state.
			...
			if (newValue != device.states.get(state, None)):
				try:
					...
					self.exceptionHandler(30, e, extraText=u"updateDeviceState: Updating device \"{}\" state: Unable to display state".format(device.name))
	...
	def rebuildDevice(self, device):
		...
		if device.deviceTypeId in kLightDeviceTypeIDs and device.pluginProps.get('modelId', u"") == u"":
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"The {} lighting device doesn't have a modelId attribute.  Adding it.".format(device.name))
			newProps['modelId'] = u""
		...
		# Prior to version 1.8.5,add option to log button press changes etc on/off, default is on 
		if "logChanges" not in newProps: 
			newProps['logChanges'] = True # 
		...
		if device.deviceTypeId in kLightDeviceTypeIDs and device.configured:
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} lighting device properties.".format(device.name))
			...
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} group device.".format(device.name))
			newProps['SupportsColor'] = True
			newProps['SupportsRGB'] = True
			newProps['SupportsWhite'] = True
			newProps['SupportsWhiteTemperature'] = True
			...
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} motion sensor device.".format(device.name))
			...
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} temperature sensor device.".format(device.name))
			...
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} light sensor device.".format(device.name))
			newProps['SupportsOnState'] = False
			newProps['SupportsSensorValue'] = True
			newProps['SupportsBatteryLevel'] = True
		elif device.deviceTypeId in kSwitchTypeIDs and device.configured:
			if self.decideMyLog(u"UpdateIndigoDevices"): self.indiLOG.log(10,u"Verifying the {} switch device.".format(device.name))
			newProps['SupportsOnState'] = True
			newProps['SupportsSensorValue'] = False
	...
	def getHueConfig(self):
		...
		for hubNumber in self.ipAddresses:
			...
			try:
				...
				self.exceptionHandler(30, e, extraText=u"Unable to obtain the configuration from the Hue bridge.{}".format(hubNumber))
		return
	...
	def updateTheTypeList(self, theType):
		...
		for hubNumber in self.ipAddresses:
			...
			try:
				...
				if isinstance(responseData, dict):
					#if self.decideMyLog(u"Special") and theType =="groups": self.indiLOG.log(10,u"Loaded {} list - {}".format(theType, json.dumps(responseData, sort_keys=True, indent=2)) )
					...
					if len(self.hueConfigDict[hubNumber][theType]) > lastCount[hubNumber] and lastCount[hubNumber] is not 0:
						...
						if countChange == 1:
							self.indiLOG.log(20,u"{} less Hue {} was found on the Hue bridge. Check your Hue Lights Indigo devices. One of them may have been controlling the missing Hue group.".format(theType, countChange) )
						elif countChange !=0:
							self.indiLOG.log(20,u"{} fewer Hue {} were found on the Hue bridge. Check your Hue Lights Indigo devices. Some of them may have been controlling the missing Hue groups.".format(theType, countChange) )
					# Make sure the plugin knows it's actually paired now.
					self.paired[hubNumber] = True
					...
					if errorDict is not None:
						...
						# Is this a link button not pressed error?
						if errorCode == 1:
							self.doErrorLog(u"update{}List: Not paired with the Hue bridge. Press the middle button on the Hue bridge, then press the Pair Now button in the Hue Lights Configuration window (Plugins menu).".format(theType))
							self.paired[hubNumber] = False

						else:
							self.doErrorLog(u"Error #{} from Hue bridge when loading available {}. Description is \"{}\".".format(errorCode, errorDict.get('description', u"(no description"),theType))
							self.paired[hubNumber] = False

			except requests.exceptions.Timeout:
				self.doErrorLog(u"Failed to load {} list from the Hue bridge at {} after {} seconds - check settings and retry.".format(theType, ipAddress, kTimeout))

			except requests.exceptions.ConnectionError:
				self.doErrorLog(u"Failed to connect to the Hue bridge at {}. - Check that the bridge is connected, turned on and the network settings are correct.".format(ipAddress))
				return

			except Exception as e:
				self.exceptionHandler(30, e, extraText=u"Unable to obtain list from the bridge {}".format(hubNumber))
		return 
		...
		if device.deviceTypeId != "hueOnOffDevice":
			...
			if rampRate == -1:
				try:
					...
					self.exceptionHandler(30, e, extraText=u"Default ramp rate could not be obtained")
					rampRate = 5
			else:
	...
	def doRGB(self, device, red, green, blue, rampRate=-1, showLog=True):
		...
		if rampRate == -1:
			try:
				...
				self.exceptionHandler(40, e, extraText=u"Default ramp rate could not be obtained")
				rampRate = 5
		else:
			rampRate = int(round(float(rampRate) * 10))
	...
	def doEffect(self, device, effect, showLog=True):
		...
		if self.decideMyLog(u"SendCommandsToBridge") or self.decideMyLog(u"Special"): self.indiLOG.log(10,u"data to be send:{}".format(requestData) )
		# Create the command based on whether this is a light or group device.
		if device.deviceTypeId == "hueGroup":
			command = "http://{}/api/{}/groups/{}/action".format(ipAddress, self.hostIds[hubNumber], groupId)
		else:
			command = "http://{}/api/{}/lights/{}/state".format(ipAddress, self.hostIds[hubNumber], bulbId)
		if self.decideMyLog(u"SendCommandsToBridge") or self.decideMyLog(u"Special"): self.indiLOG.log(10,u"URL: " + command)
		try:
			r = requests.put(command, data=requestData, timeout=kTimeout, headers={'Connection':'close'})
		except requests.exceptions.Timeout:
			self.doErrorLog(u"Failed to connect to the Hue bridge at {} after {} seconds. - Check that the bridge is connected and turned on." .format(ipAddress, kTimeout))
		...
		elif self.decideMyLog(u"ReadFromBridge") or self.decideMyLog(u"Special"):	self.indiLOG.log(10,u"Got response - {}".format(r.content) )
		# Log the change (if enabled).
		if showLog:
	...
	def updateAllHueLists(self):
	   ...
	   # Do we have a unique Hue username (a.k.a. key or host ID)?
		...
		if self.pluginState == "init": self.indiLOG.log(20,u"hostIds read:  {}".format(self.hostIds))
		...
		for hubNumber in  self.hostIds:
			...
			elif self.pluginState == "init":
				self.indiLOG.log(10,u"The username Hue Lights uses to connect to the Hue bridge is {}" .format(hueUsername) )
		...
		for hubNumber in tempIP:
			...
			if not self.isValidIP(tempIP[hubNumber]):
				self.doErrorLog(u"IP address\"{}\" set for the Hue bridge is not valid, please set in config".format(tempIP[hubNumber]))
				return

			self.ipAddresses[hubNumber] = tempIP[hubNumber]
		if self.pluginState == "init": 
			self.indiLOG.log(20,u"ipAddresses loaded:{} ".format(self.ipAddresses))
		...
		self.getHueConfig()
		if self.pluginState == "init": 
			self.indiLOG.log(20,u"bridges paired:  {}".format(self.paired))
			for hubNumber in self.hueConfigDict:
				self.indiLOG.log(20,u"bridge: {}  loaded: light(s): {:2d}; group(s): {:2d}; scene(s): {:2d}; user(s): {:2d};  resource(s): {:2d}; rule(s): {:3d}; schedule(s): {:2d}; sensor(s): {:2d}".format(
					hubNumber, len(self.hueConfigDict[hubNumber]['lights'] ), len(self.hueConfigDict[hubNumber]['groups'] ), len(self.hueConfigDict[hubNumber]['users'] ), len(self.hueConfigDict[hubNumber]['scenes'] ), len(self.hueConfigDict[hubNumber]['resourcelinks'] ), len(self.hueConfigDict[hubNumber]['rules'] ), len(self.hueConfigDict[hubNumber]['schedules'] ), len(self.hueConfigDict[hubNumber]['sensors'] )) )
		return
	...
	def restartPairing(self, valuesDict):
		...
		if valuesDict.get('address', "") == "":
			# The field was left blank.
			if self.decideMyLog(u"Init"): self.indiLOG.log(30,u"IP address \"{}\" is blank.".format(valuesDict['address']))
			isError = True
			errorsDict['address'] = u"The IP Address field is blank. Please enter an IP Address for the Hue bridge."
			errorsDict['showAlertText'] += errorsDict['address'] + u"\n\n"
		...
		if not isError:
			try:
				...
				self.exceptionHandler(40, e, extraText=u"Connection error")
				isError = True
				errorsDict['address'] = u"Connection error. Please check the IP address and ensure that the Indigo server and Hue bridge are connected to the network."
				errorsDict['showAlertText'] += errorsDict['address'] + u"\n\n"
		...
		if isError:
			...
			try:
				...
				self.exceptionHandler(40, e, extraText=u"Connection to {}  failed,timed out after {} seconds.".format(valuesDict['address'], kTimeout))
				isError = True
				...
				self.exceptionHandler(40, e, extraText=u"Connection to {} There was a connection error".format(valuesDict['address']))
				isError = True
				...
				self.exceptionHandler(40, e, extraText=u"Connection to {}  failed".format(valuesDict['address']))
				isError = True
				errorsDict['startPairingButton'] = u"Connection error. Please check the IP address and ensure that the Indigo server and Hue bridge are connected to the network."
				errorsDict['showAlertText'] += errorsDict['startPairingButton'] + u"\n\n"
	...
	def startStopBrightening(self, action, device):
		...
		if device.deviceTypeId == "hueOnOffDevice":
			self.doErrorLog(u"The {} device doesn't support dimming. Please edit the action and select a device that supports dimming.".format(device.name))
			return
	...
	def savePreset(self, action, device):
		...
		if rampRate != -1:	# May still be a sring if passed by embedded script call.
			try:
				...
				self.exceptionHandler(40, e, extraText=u"Invalid Ramp Rate value \"{}\"".format(rampRate))
				rampRate = -1
		else:
	...
	def recallPreset(self, action, device):
		...
		try:
			...
			self.exceptionHandler(40, e, extraText=u"Preset number {} couldn't be recalled.".format(presetId + 1))
			return
  ...
  # Bulb List Generator
	...
	def printHueData(self, valuesDict, menuItem):
		...
		if valuesDict['whatToPrint'] == "lights":
			...
			for hubNumber in sorted(self.ipAddresses):
				...
				outs.append(u"registered users (Whitelist)")
		...
		elif valuesDict['whatToPrint'] == "orphens":
			self.indiLOG.log(20,"======================= print Hue orphens =====================")
				...
				for hubNumber in self.ipAddresses:
					...
					for indigoName in indigoNameToHueId[hubNumber]:
						...
						if  hueIdToFind not in self.hueConfigDict[hubNumber][tt[1]]:
							self.indiLOG.log(20," orphen indigo {:7s} device: == {:47s} ==   ID:{:3} does not exist on bridge: {} - {}".format(tt[1], indigoName, hueIdToFind, hubNumber, self.ipAddresses[hubNumber]))
			self.indiLOG.log(20,"======================= print Hue orphens END =================")
			...
			self.indiLOG.log(20,"printHueData --- {}: {}-{}: json=\n{} ".format(whatToPrint, hubNumber, ID, json.dumps(self.hueConfigDict[hubNumber][whatToPrint][ID], indent=2, sort_keys=True)))
	...
	def getIndigoDevDict(self, typeName):
		...
		for dev in indigo.devices.iter(self.pluginId):
			if typeName in dev.pluginProps:
				hubNumber = dev.pluginProps.get('hubNumber','0')
	...
	def setDebugAreas(self, valuesDict, item=""):
		...
		for d in _debugAreas:
			self.pluginPrefs[u"debug"+d] =  valuesDict[u"debug"+d]
		self.getDebugLevels()
		return 
####-----------------	 ---------
...
####----------------- print to log as error only if different from last error message ---------
	def doErrorLog(self, errorText, level=40):
	...
	def exceptionHandler(self, level, exception_error_message, extraText=""):
		...
		try:
			...
			if extraText !="":
				...
				log_message +=  "\n{} @line {}: '{}'\n{}".format(method, line_number, statement, extraText)
			if self.lastErrorMessage == log_message: return 
			self.lastErrorMessage = log_message
